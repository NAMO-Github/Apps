...
const NFTCardWithMetaDataParser: FC<NFTCardProps> = ({
  lenderName,
  avatarUrl,
  isMyNFT,
  isMyGallery = false,
  ...
  const [nftInfo, setNftInfo] = useState(detailNft);
  const handleParseNFTMoralist = useCallback(async (data: InfoNFT) => {
    const newNFT = await parseMetaDataToMoralis(data);
    setNftInfo(newNFT);
  }, []);
  const title = nftInfo?.metaData?.name || nftInfo?.name || 'Unnamed';
  useEffect(() => {
    handleParseNFTMoralist(detailNft);
  }, [handleParseNFTMoralist, detailNft]);
...
export default memo(NFTCardWithMetaDataParser);
